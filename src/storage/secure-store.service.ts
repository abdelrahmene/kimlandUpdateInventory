...
export class SecureStoreService {
  ...
  private encrypt(text: string): EncryptedData {
    ...
    const cipher = crypto.createCipherGCM('aes-256-gcm', this.ENCRYPTION_KEY, iv);
  ...
  private decrypt(encryptedData: EncryptedData): string {
    const decipher = crypto.createDecipherGCM('aes-256-gcm', this.ENCRYPTION_KEY, Buffer.from(encryptedData.iv, 'hex'));
